...
class LayoutTest {
    ...
    fun actualSize_twoCards_cr80_onA4_centeredStack() {
        ...
        // stack height = 2*54 + 10 = 118; topY = (297-118)/2 = 89.5; x = (210-85.6)/2 = 62.2
        assertEquals(2, result.cards.size)
        assertRect(LayoutRect(62.2, 89.5, 85.6, 54.0), result.cards[0])
        assertRect(LayoutRect(62.2, 153.5, 85.6, 54.0), result.cards[1])
    ...
    fun fitWidth_twoCards_scaledToContentWidth_andCanvasCroppedToContent() {
        ...
        assertRect(LayoutRect(6.0, 6.0 + h + 10.0, contentW, h), result.cards[1])
        // canvas height cropped to content: margins + 2 cards + gap
        assertEquals(2 * h + 10.0 + 12.0, result.pageHeightMm, tol)
        assertEquals(210.0, result.pageWidthMm, tol)
    ...
    fun fitPage_twoTallCards_uniformScaleIncludingGap_noOverflow() {
        ...
        // Reference stack (each at usable width 198): refH = 198/0.7 each; refStack = 2*refH + gap 10.
    ...
    fun actualSize_perSideSizes_differentOrientations_centeredOnFullPage() {
        ...
        // stack height = 54 + 10 + 85.6 = 149.6; topY = (297-149.6)/2 = 73.7
        assertRect(LayoutRect((210.0 - 85.6) / 2.0, 73.7, 85.6, 54.0), result.cards[0])
        assertRect(LayoutRect((210.0 - 54.0) / 2.0, 73.7 + 54.0 + 10.0, 54.0, 85.6), result.cards[1])
        assertEquals(297.0, result.pageHeightMm, tol)
    }
    ...
    fun actualSize_cropToContent_tightCanvas() {
        ...
        // canvas = content + margins(6): width = 85.6 + 12 = 97.6; height = (54+10+54) + 12 = 130
        assertEquals(97.6, result.pageWidthMm, tol)
        assertEquals(130.0, result.pageHeightMm, tol)
        assertRect(LayoutRect(6.0, 6.0, 85.6, 54.0), result.cards[0])
        assertRect(LayoutRect(6.0, 6.0 + 54.0 + 10.0, 85.6, 54.0), result.cards[1])
    }

    @Test
    fun actualSize_twoCr80Portrait_fitsA5_noOverflow() {
        // Spec check: two CR-80 portrait cards (54 x 85.6) stack ~181.2 mm < A5 height 210 mm.
        val a5 = PaperSize.A5
        val result = LayoutCalculator.calculate(
            LayoutInput(
                fitMode = FitMode.ACTUAL_SIZE,
        ...
        val stackHeight = 85.6 + 10.0 + 85.6 // 181.2
        val topY = (a5.heightMm - stackHeight) / 2.0
        assertEquals(14.4, topY, tol)
